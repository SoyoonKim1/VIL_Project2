...
def feature_parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "If set, load official labels from torchvision dataset split "
            "(stl10: train/test, cifar10: train/test). "
            "In this mode, provide --train-features and --test-features only."
        ),
    )
...
def load_official_dataset_labels(dataset_name, data_root, download):
    if dataset_name == "stl10":
        train_ds = STL10(root=data_root, split="train", download=download)
        test_ds = STL10(root=data_root, split="test", download=download)
        train_labels = torch.as_tensor(np.asarray(train_ds.labels), dtype=torch.long)
        test_labels = torch.as_tensor(np.asarray(test_ds.labels), dtype=torch.long)
    elif dataset_name == "cifar10":
        train_ds = CIFAR10(root=data_root, train=True, download=download)
        test_ds = CIFAR10(root=data_root, train=False, download=download)
        train_labels = torch.as_tensor(np.asarray(train_ds.targets), dtype=torch.long)
        test_labels = torch.as_tensor(np.asarray(test_ds.targets), dtype=torch.long)
    else:
...
def feature_main():
    ...
    if args.dataset is not None:
        if args.train_features is None or args.test_features is None:
            raise RuntimeError(
                "Dataset mode requires --train-features and --test-features."
            )
        if args.train_labels is not None or args.test_labels is not None:
            raise RuntimeError(
                "In dataset mode, do not pass --train-labels/--test-labels. "
                "Labels are loaded from official dataset splits."
            )
        train_features = load_array(args.train_features).float()
        test_features = load_array(args.test_features).float()
        train_labels, test_labels = load_official_dataset_labels(
            args.dataset,
            data_root=args.data_root,
            download=args.download,
        )
        if train_features.ndim != 2 or test_features.ndim != 2:
            raise RuntimeError("In dataset mode, feature files must be 2D arrays with shape (N, D).")
        if train_features.size(0) != train_labels.size(0):
            raise RuntimeError(
                f"{args.dataset}: train feature length mismatch: "
                f"{train_features.size(0)} vs {train_labels.size(0)}"
            )
        if test_features.size(0) != test_labels.size(0):
            raise RuntimeError(
                f"{args.dataset}: test feature length mismatch: "
                f"{test_features.size(0)} vs {test_labels.size(0)}"
            )
        jobs.append((args.dataset, train_features, train_labels, test_features, test_labels))
